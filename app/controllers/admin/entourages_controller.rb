module Admin
  class EntouragesController < Admin::BaseController
    ...
    def index_params
      params.permit([:search, :moderator_id, q: [:entourage_type_eq, :status_in, :display_category_eq, :country_eq, :postal_code_start, :pin_eq, :group_type_eq, :moderation_action_outcome_blank, postal_code_start_any: [], postal_code_not_start_all: []]]).to_h
    end

    def entourage_params
      metadata_keys = params.dig(:entourage, :metadata).try(:keys) || [] # security issue
      metadata_keys -= [:starts_at]
      permitted = params.require(:entourage).permit(:group_type, :status, :title, :description, :category, :entourage_type, :display_category, :latitude, :longitude, :public, :online, :url, :event_url, :user_id, :entourage_image_id, :invite_followers, :change_ownership_message, pins: [], metadata: metadata_keys)
