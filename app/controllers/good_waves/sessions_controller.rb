module GoodWaves
  class SessionsController < BaseController
    skip_before_action :authenticate_user!, only: [:new, :identify, :authenticate, :reset_password]
    skip_before_action :ensure_profile_complete!
    # layout_options(menu: false, partner_name: false, exit_admin: false)

    def new
      if current_user
        return redirect_to good_waves_path
      end
    end
    ...
    def authenticate
      ...
      if user.nil?
        redirect_to identify_good_waves_session_path(
          phone: params[:phone],
      ...
      redirect_to params[:continue].presence || good_waves_path
    end
