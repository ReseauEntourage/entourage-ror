class PushNotificationTriggerJob
  ...
  def perform method, id, changes = {}
    return unless self.class.respond_to?(method)

    self.class.send(method, id, changes)
  rescue => e
    puts "ERROR: #{e.message}"
    puts "ERROR: #{e.backtrace.join("\n")}"
  end

  def self.perform_later method, id, changes = {}
    PushNotificationTriggerJob.perform_async(method, id, changes)
  end

  class << self
    def neighborhoods_entourage_on_create id, changes
      neighborhoods_entourage = NeighborhoodsEntourage.find(id)
    ...
    def entourage_on_create id, changes
      entourage = Entourage.find(id)
    ...
    def entourage_on_update id, changes
      entourage = Entourage.find(id)

      return outing_on_update(entourage, changes) if entourage.outing?
    end

    def outing_on_update outing, changes
      return outing_on_update_status(outing, changes) if changes.keys.include?("status")
      ...
      notify(instance: outing, users: users, params: {
        object: outing.title,
        content: update_outing_message(outing, changes)
      })
    end

    def outing_on_update_status outing, changes
    ...
    def chat_message_on_create id, changes
    ...
    def join_request_on_create id, changes
      join_request = JoinRequest.find(id)
    ...
    def join_request_on_update id, changes
      join_request = JoinRequest.find(id)

      return unless changes.keys.include?("status")
      return join_request_on_create(join_request) unless changes["status"]&.first&.to_sym == :pending
