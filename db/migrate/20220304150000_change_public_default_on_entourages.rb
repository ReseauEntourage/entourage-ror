class ChangePublicDefaultOnEntourages < ActiveRecord::Migration[5.2]
  def up
    drop_view_entourages_extended

    change_column :entourages, :public, :boolean, default: true

    create_view_entourages_extended
  end

  def down
    drop_view_entourages_extended

    change_column :entourages, :public, :boolean, default: false

    create_view_entourages_extended
  end

  def drop_view_entourages_extended
    execute <<-SQL
      drop view entourages_extended
    SQL
  end

  def create_view_entourages_extended
    execute <<-SQL
      create or replace view entourages_extended as
        select
          ...
          case
            ...
            when substring(postal_code for 2) = '93' then 'Seine-Saint-Denis (93)'
            else                                          'hors zone'
          end as zone,
