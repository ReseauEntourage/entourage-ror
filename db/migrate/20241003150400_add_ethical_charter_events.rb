class AddEthicalCharterEvents < ActiveRecord::Migration[6.1]
  disable_ddl_transaction!

  def up
    execute <<-SQL
      alter type event_name add value 'onboarding.chat_messages.ethical_charter.sent'
    SQL
  end

  def down
    Event.where(name: 'onboarding.chat_messages.ethical_charter.sent').delete_all
