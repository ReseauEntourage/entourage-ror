class RemoveGoalEvent < ActiveRecord::Migration[7.1]
  disable_ddl_transaction!

  def up
    Event.where(name: 'onboarding.profile.goal.entered').delete_all
  ...
  def down
    execute <<-SQL
      alter type event_name add value 'onboarding.profile.goal.entered'
    SQL
  end
end
