...
describe Admin::AnnouncementsController do
  ...
  describe 'GET #index' do
    let!(:announcement_list) { FactoryGirl.create_list(:announcement, 2, user_goals: [:offer_help, :ask_for_help], areas: [:dep_75, :hors_zone], id: [1, 2]) }
    ...
    context "has announcements with good filtering" do
      before { get :index, moderator_id: :any, user_goal: "offer_help", area: "dep_75" }

      it { expect(assigns(:announcements)).to match_array(announcement_list) }
    end

    context "has no announcements when user_goal does not match" do
      before { get :index, moderator_id: :any, user_goal: "organization", area: "dep_75" }

      it { expect(assigns(:announcements)).to eq([]) }
    end

    context "has no announcements when area does not match" do
      before { get :index, moderator_id: :any, user_goal: "offer_help", area: "sans_zone" }

      it { expect(assigns(:announcements)).to eq([]) }
    end

    context "has announcements when area does not match but announcements are hors_zone" do
      before { get :index, moderator_id: :any, user_goal: "offer_help", area: "dep_92" }
