...
RSpec.describe Api::V0::TourPointsController, :type => :controller, skip: true do
  render_views
  ...
  describe "POST create" do
    ...
    context "with inexisting tour" do
      it { expect {
            post 'create', params: { tour_id: 0, token: user.token, tour_points: [{latitude: tour_point.latitude, longitude: tour_point.longitude, passing_time: tour_point.passing_time.iso8601(3)}], format: :json }
    ...
    context "with missing passing time" do
      it "doesn't raise exception" do
        expect {
          post 'create', params: { tour_id: tour.id, token: user.token, tour_points: {latitude: 1.0, longitude: 2.0} }
        }.to_not raise_error
