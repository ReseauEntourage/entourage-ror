...
RSpec.describe Api::V0::UsersController, :type => :controller, skip: true do
  ...
  describe 'POST #login' do
    context 'when the user exists' do
      ...
      context 'when the phone number and sms code are valid' do
        before { post 'login', params: { phone: user.phone, sms_code: "123456", device_id: device_id, device_type: device_type, format: 'json' } }
      ...
      context 'when sms code is invalid' do
        before { post 'login', params: { phone: user.phone, sms_code: 'wrong sms code', device_id: device_id, device_type: device_type, format: 'json' } }
    ...
    context 'when user does not exist' do
      context 'using the email' do
        before { post 'login', params: { email: 'not_existing@nowhere.com', sms_code: 'sms code', format: 'json' } }
        it { expect(response.status).to eq(401) }
        it { expect(assigns(:user)).to be_nil }
      end
      context 'using the phone number and sms code' do
        before { post 'login', params: { phone: 'phone', sms_code: 'sms code', format: 'json' } }
        it { expect(response.status).to eq(401) }
        it { expect(assigns(:user)).to be_nil }
      end
    end
  ...
  describe '#updateme' do
    context 'authentication is OK' do
      ...
      context 'params are valid' do
        before { patch 'update_me', params: { token:user.token, user: { email:'new@e.mail', sms_code:'654321' }, format: :json } }
        it { expect(response.status).to eq(200) }
      ...
      context 'params are invalid' do
        before { patch 'update_me', params: { token:user.token, user: { email:'bademail', sms_code:'badcode' }, format: :json } }
        it { expect(response.status).to eq(400) }
      end
    end

    context 'bad authentication' do
      before { patch 'update_me', params: { token:'badtoken', user: { email:'new@e.mail', sms_code:'654321' }, format: :json } }
      it { expect(response.status).to eq(401) }
    end
  end
  ...
  describe 'code' do
    ...
    describe "unknown phone" do
      it "returns 404" do
        expect {
          patch 'code', params: { id: "me", user: { phone: "0000" }, code: {action: "regenerate"}, format: :json }
