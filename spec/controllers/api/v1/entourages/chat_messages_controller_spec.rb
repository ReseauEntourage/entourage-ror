    ...
    context "signed in" do
      ...
      describe "send push notif" do
        it "sends notif to everyone accaepted except message sender" do
          ...
          expect_any_instance_of(PushNotificationService).to receive(:send_notification).with("John D", 'Nouveau message', 'foobar', [join_request2.user], {:joinable_id=>entourage.id, :joinable_type=>"Entourage", :group_type=>'action', :type=>"NEW_CHAT_MESSAGE"})
