      ...
      context "inviter accepted in entourage" do
        ...
        context "invitation already exists" do
          context "user has already connected to entourage" do
            ...
            before { expect_any_instance_of(PushNotificationService).to receive(:send_notification).with("John D.", "Foobar", "John D. vous invite à rejoindre une action.", [existing_user], {type: "ENTOURAGE_INVITATION", entourage_id: entourage.id, group_type: 'action', inviter_id: user.id, invitee_id: existing_user.id, invitation_id: 123}) }
