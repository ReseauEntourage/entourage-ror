...
describe Api::V1::Neighborhoods::ChatMessages::SurveyResponsesController do
  let(:user) { create(:pro_user) }
  let(:neighborhood) { create :neighborhood }
  let(:survey) { create(:survey) }
  let(:chat_message) { create(:chat_message, messageable: neighborhood, survey: survey) }
  ...
  describe 'create' do
    let(:request) { post :create, params: { neighborhood_id: neighborhood.to_param, chat_message_id: chat_message.id, token: user.token, responses: [true, false] } }
