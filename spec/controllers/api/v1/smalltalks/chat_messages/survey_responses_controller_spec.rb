...
describe Api::V1::Smalltalks::ChatMessages::SurveyResponsesController do
  let(:user) { create(:pro_user) }
  let(:smalltalk) { create :smalltalk }
  let(:survey) { create(:survey) }
  let(:chat_message) { create(:chat_message, messageable: smalltalk, survey: survey) }
  ...
  describe 'create' do
    let(:request) { post :create, params: { smalltalk_id: smalltalk.to_param, chat_message_id: chat_message.id, token: user.token, responses: [true, false] } }
