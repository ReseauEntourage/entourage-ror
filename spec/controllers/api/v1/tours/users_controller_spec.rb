...
describe Api::V1::Tours::UsersController do
  ...
  describe 'POST create' do
    context "not signed in" do
      before { post :create, params: { tour_id: tour.to_param } }
  ...
  describe "PATCH update" do
    context "not signed in" do
      before { patch :update, params: { tour_id: tour.to_param, id: user.id } }
      it { expect(response.status).to eq(401) }
    end
    ...
    context "signed in as accepted member of the tour" do
      ...
      context "invalid params" do
        before { patch :update, params: { tour_id: tour.to_param, id: requester.id, status: "foobar", token: user.token } }
        it { expect(response.status).to eq(400) }
        it { expect(tour_requester.reload.status).to eq("pending") }
      end
    end
    ...
    context "user didn't request to join tour" do
      it "raises not found" do
        expect {
          patch :update, params: { tour_id: tour.to_param, id: user.id, user: {status: "accepted"}, token: user.token }
  ...
  describe "DELETE destroy" do
    context "not signed in" do
      before { delete :destroy, params: { tour_id: tour.to_param, id: user.id } }
      it { expect(response.status).to eq(401) }
    end
    ...
    context "delete the author of the tour" do
      ...
      let!(:tour_member) { create(:join_request, user: user, joinable: tour, status: "accepted") }
      before { delete :destroy, params: { tour_id: tour.to_param, id: tour.user.id, token: user.token } }
      it { expect(response.status).to eq(400) }
    ...
    context "user didn't request to join tour" do
      it "raises not found" do
        expect {
          delete :destroy, params: { tour_id: tour.to_param, id: user.id, token: user.token }
        }.to raise_error(ActiveRecord::RecordNotFound)
      end
    end
  end
