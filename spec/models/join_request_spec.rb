...
RSpec.describe JoinRequest, type: :model do
  ...
  describe "after_save join_request" do
    ...
    describe "no requested_at for accepted" do
      let(:join_request) { create :join_request, joinable: conversation, status: 'accepted', message: 'message' }
    ...
    describe "requested_at for pending" do
      let(:join_request) { create :join_request, joinable: conversation, status: 'pending', message: 'message' }
    ...
    describe "no requested_at for pending with nil message" do
      let(:join_request) { create :join_request, joinable: conversation, status: 'pending', message: nil }
    ...
    describe "from pending to accepted" do
      let!(:join_request_past) { create :join_request, joinable: conversation, status: 'pending', message: 'message', requested_at: 6.hours.ago }
      let(:join_request)       { create :join_request, joinable: conversation, status: 'pending', message: 'message' }
