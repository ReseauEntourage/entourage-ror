...
RSpec.describe LoginHistory, :type => :model do
  ...
  describe "unique by hour" do
    let!(:previous_login) { LoginHistory.create!(user_id: 1, connected_at: DateTime.parse("10/10/2010 21:15:00").utc) }
    ...
    it { expect(LoginHistory.new(user_id: 1, connected_at: DateTime.parse("10/10/2010 20:44:00").utc).save).to be true }
    it { expect(LoginHistory.new(user_id: 1, connected_at: DateTime.parse("09/10/2010 21:44:00").utc).save).to be true }
  end
end