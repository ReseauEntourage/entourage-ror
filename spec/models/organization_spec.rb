...
RSpec.describe Organization, type: :model do
  ...
  it "should count encounters" do
    ...
    tour         = FactoryBot.create(:tour, user: user)
    FactoryBot.create_list(:encounter, 4, tour: tour)
  ...
  it "should count tours" do
    organization  = FactoryBot.create(:organization)
    user          = FactoryBot.create(:pro_user, organization_id: organization.id)
    FactoryBot.create_list(:tour, 3, user: user)
  ...
  it "should count last month active members" do
    organization  = FactoryBot.create(:organization)
    FactoryBot.create_list(:user, 6, organization: organization, last_sign_in_at: Time.current - 5.days)
  ...
  it "should retrieve last tour date" do
    organization = FactoryBot.create(:organization)
    user         = FactoryBot.create(:pro_user, organization_id: organization.id)
    FactoryBot.create_list(:tour, 6, user: user)
