...
RSpec.describe UserApplication, type: :model do
  it { expect(FactoryGirl.build(:user_application, device_family: UserApplication::IOS).save).to be true }
  ...
  it "can have multiple application version per user and device os" do
    ...
    expect(FactoryGirl.build(:user_application, user: user, device_family: UserApplication::IOS, version: "1.0").save).to be true
    expect(FactoryGirl.build(:user_application, user: user, device_family: UserApplication::IOS, version: "1.0").save).to be true
    expect(FactoryGirl.build(:user_application, user: user, device_family: UserApplication::ANDROID, version: "1.0").save).to be true
    expect(FactoryGirl.build(:user_application, user: user, device_family: UserApplication::IOS, version: "1.1").save).to be true
  end

  it "has unique push token" do
    expect(FactoryGirl.build(:user_application, push_token: "foo", device_family: UserApplication::IOS).save).to be true
    expect(FactoryGirl.build(:user_application, push_token: "foo", device_family: UserApplication::IOS).save).to be false
    expect(FactoryGirl.build(:user_application, push_token: "bar", device_family: UserApplication::IOS).save).to be true
  end
