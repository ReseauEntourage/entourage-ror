...
describe CleanupService, type: :service do
  describe '.force_close_tours' do
    ...
    it { expect(Tour.find(tour1.id).email_sent).to be true }
    ...
    it { expect(Tour.find(tour2.id).email_sent).to be true }
    ...
    it { expect(Tour.find(tour3.id).email_sent).to be false }
