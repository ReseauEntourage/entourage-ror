...
describe Onboarding::ChatMessagesService, type: :service do
  describe '.deliver_welcome_message' do
    ...
    describe 'one user has already a conversation; that should not skip the other one' do
      ...
      before { Onboarding::ChatMessagesService.stub(:welcome_message_user_ids) { [user.id, other_user.id] } }
      it { expect { subject }.to change { ChatMessage.count }.by(1) }
    end
  ...
  describe '.deliver_ethical_charter' do
    let(:run_time) { 1.week.from_now.monday.change(hour: 10, minute: rand(60)) }
    let(:welcome_sent_time) { run_time.advance(seconds: -rand(6.hours..7.hours)) }
    let!(:admin) { create :admin_user, first_name: 'foobar' }
    let(:moderation_area) { create :moderation_area, moderator: admin }
    let!(:user) { create :public_user, first_name: 'lily-rose' }
    let(:address) { build :address }

    before do
      Onboarding::UserEventsTracking.stub(:enable_tracking?) { true }
      ModerationServices.stub(:moderator_for_user) { admin }
      Timecop.freeze(welcome_sent_time) do
    ...
    subject { Timecop.freeze(run_time) { Onboarding::ChatMessagesService.deliver_ethical_charter } }

    it { expect { subject }.to change { ChatMessage.count }.by(1) }

    describe 'event tracking' do
      def event
        Event.where(name: 'onboarding.chat_messages.ethical_charter.sent', user_id: user.id).first
      end

      it { expect { subject }.to change { event.present? }.to true }
    end

    describe 'chat message content' do
      before { subject }
      let(:message) { ChatMessage.last }

      it { expect(message.user).to eq admin }
      it { expect(message.content).to include('foobar') }
    end
    ...
    describe 'user has already received the ethical charter message' do
      before { Event.track('onboarding.chat_messages.ethical_charter.sent', user_id: user.id) }
      it { expect { subject }.not_to change { ChatMessage.count } }
    end

    describe 'another user in the same batch should still receive the message' do
      let(:other_address) { build :address }
      let!(:other_user) { create :public_user, first_name: 'Jack', goal: :offer_help, address: other_address }

      before { Onboarding::ChatMessagesService.stub(:ethical_charter_user_ids) { [user.id, other_user.id] } }
