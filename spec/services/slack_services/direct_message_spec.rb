...
describe SlackServices::DirectMessage do
  describe '#send!' do
    ...
    context 'when SLACK_DM_USER_TOKEN is not configured' do
      let(:user) { create(:pro_user, slack_id: 'U123') }

      before { allow(ENV).to receive(:[]).and_call_original }
      before { allow(ENV).to receive(:[]).with('SLACK_DM_USER_TOKEN').and_return(nil) }
    ...
    context 'with a slack_id and a configured token' do
      ...
      before { allow(ENV).to receive(:[]).and_call_original }
      before { allow(ENV).to receive(:[]).with('SLACK_DM_USER_TOKEN').and_return('xoxp-test') }
